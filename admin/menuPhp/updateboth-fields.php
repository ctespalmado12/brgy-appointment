...
if(isset($_POST['displaySend'])){
    ...
    $transac = $_POST['resTransac'];
   


    if($_POST['resRange'] == "All"){
        ...
        where app.reason_for_appointment = '$transac' 
        
        ";
    
    }
  else if($_POST['resRange'] == 'Daily'){
    ...
    JOIN client cl ON cl.client_id = app.client_id
    JOIN brgy_official_schedule as brgyOffSched ON brgyOffSched.brgy_official_schedule_id = app.brgy_official_schedule_id 
    where app.reason_for_appointment = '$transac' AND brgyOffSched.schedule_date = CURDATE()

    
    ";
    }
  else if($_POST['resRange'] == 'Weekly'){
    $sql = "Select CONCAT(first_name,' ',last_name) 
    as name, brgyOffSched.schedule_date as appdate,cl.client_id as client_id, app.appointment_id as appointment_id, cl.first_name as firstname,  broff.fullname as brgy_name, app.appointment_time as apptime, app.reason_for_appointment as reason
    FROM appointment app
    JOIN brgy_official broff ON broff.brgy_official_id = app.brgy_official_id
    JOIN client cl ON cl.client_id = app.client_id
    JOIN brgy_official_schedule as brgyOffSched ON brgyOffSched.brgy_official_schedule_id = app.brgy_official_schedule_id 
    where app.reason_for_appointment = '$transac' AND brgyOffSched.schedule_date BETWEEN CURDATE()-7 AND CURDATE()
  ...
  else if($_POST['resRange'] ==  'Monthly'){
    $sql = "Select CONCAT(first_name,' ',last_name) 
    as name, brgyOffSched.schedule_date as appdate,cl.client_id as client_id, app.appointment_id as appointment_id, cl.first_name as firstname,  broff.fullname as brgy_name, app.appointment_time as apptime, app.reason_for_appointment as reason
    FROM appointment app
    JOIN brgy_official broff ON broff.brgy_official_id = app.brgy_official_id
    JOIN client cl ON cl.client_id = app.client_id
    JOIN brgy_official_schedule as brgyOffSched ON brgyOffSched.brgy_official_schedule_id = app.brgy_official_schedule_id 
    where app.reason_for_appointment = '$transac' AND   brgyOffSched.schedule_date >= DATE_SUB(CURDATE(), INTERVAL 1 MONTH)
  ...
  else if($_POST['resRange'] == 'Annually'){
    //brgyOffSched.schedule_date >= DATE_SUB(CURDATE(), INTERVAL 1 YEAR)
    $sql = "Select CONCAT(first_name,' ',last_name) 
    as name, brgyOffSched.schedule_date as appdate,cl.client_id as client_id, app.appointment_id as appointment_id, cl.first_name as firstname,  broff.fullname as brgy_name, app.appointment_time as apptime, app.reason_for_appointment as reason
    FROM appointment app
    JOIN brgy_official broff ON broff.brgy_official_id = app.brgy_official_id
    JOIN client cl ON cl.client_id = app.client_id
    JOIN brgy_official_schedule as brgyOffSched ON brgyOffSched.brgy_official_schedule_id = app.brgy_official_schedule_id 
    where app.reason_for_appointment = '$transac' AND  YEAR (brgyOffSched.schedule_date)= YEAR (CURDATE());
    ...
    // JOIN client cl ON cl.client_id = app.client_id
    // JOIN brgy_official_schedule as brgyOffSched ON brgyOffSched.brgy_official_schedule_id = app.brgy_official_schedule_id 
    // where app.reason_for_appointment = '$transac' AND brgyOffSched.schedule_date = 
